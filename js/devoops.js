...
function LoadBootstrapDateTimePicker(callback){
	if (!$.fn.bootstrapDateTimePicker){
		$.getScript('plugins/bootstrap_datetimepicker_master/src/js/bootstrap-datetimepicker.js', callback);
	}
	else {
...
function OpenModalBox(header, inner, bottom){
	...
	// $('body').addClass('expanded-padding');
}
...
function ajaxdaw(id){
	...
	$.ajax({  
		type: "POST", 
		url: "ajax/ajax.php",  
		data: { id: id, action: "employee_table" },
		success: function(id) { 
			// console.log(id);
			$('.asdf').html('')
			$('.asdf').html(id)
		...
		$('#defaultForm').bootstrapValidator({
			...
			fields: {
				...
				text_cls:{
					validators: {
						notEmpty: {
							message: 'Please Select Clause(s)'
						}
					}
				},
				text_rem:{
					validators:{
						notEmpty: {
							message: 'The Remarks field is required and can\'t be empty'
						},
						stringLength: {
							min: 2,
							max: 3000,
			...
			submitHandler: function(form) {
			   $.ajax({
		            ...
		            success: function(msg) {
		                ...
		                if (msg=="success") {
		                	alert('Finding Encoded Successfully.')
		                	LoadAjaxContent('ajax/add_af.php')
		                };
		            }
	            });   
			}

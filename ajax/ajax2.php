...
if ($action=="update_user_db1" OR $action=="update_user_db2") {
	...
	$query = "UPDATE `user` SET `emp_group`='$grp_upd', `userroles`=$user_upd WHERE p_id=$id_f";
	$result = mysql_query($query) or die($query . "<br/><br/>" . mysql_error());
	if($result){
		echo "Success";
	}else{
...
if ($action=="hk_add_budget") {
	...
	$r_query = "SELECT * from fp_hks_tbl WHERE loc=$loc AND mos=$mos";
	...
	if ($row>0) {
		...
		$query  = " INSERT INTO `fp_hks_tbl` (`id`, `loc`, `dept`, `budget`, `actual_budget`, `mos`, `yr`, `date_encoded`, `is_deleted` )
   			  	VALUES ('', '$loc' , '$dept', '$budget' , 0 , '$mos' , 1 , NOW() , 0)";
	  	$result = mysql_query($query);
	...
	$tab = '<table class="pck_table_add table table-striped table-bordered table-hover table-heading no-border-bottom table-vcenter" style="font-size: 12px;">
		...
		<tbody id="f_pck_table">';
				...
				while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
					...
					$pck_query = "SELECT * FROM fp_pck_tbl WHERE empid=$eid AND mos=$mos";
...
if ($action=="fin_gen"){	
	...
	mysql_query("DELETE FROM `pms_db_final` WHERE `mos`=$mos AND pers_id=1");
	...
	while($row = mysql_fetch_array($result, MYSQL_ASSOC)){
		...
		$query2 	= "SELECT * FROM fp_pck_tbl WHERE empid=$eid AND mos=$mos";
		$result2 	= mysql_query($query2);
		...
		if ($x==1) {
			...
			while($row2 = mysql_fetch_array($result2, MYSQL_ASSOC)){
				...
				$f_query  = " INSERT INTO `pms_db_final` (`emp_id`, `yr`, `mos`, `pers_id`, `dim_id`, `pts`, `ratng`, `rmks`  )
	   			  			VALUES ('$eid', 2014 , '$mos', '$pers_id' , '$dim_id' , '$pts' , '$pck_g', '$pck_rmk')";
		...
		$query3 = "SELECT * FROM fp_hks_tbl WHERE mos=$mos AND loc=$loc AND dept=4";
		...
		while($row3 = mysql_fetch_array($result3, MYSQL_ASSOC)){
			...
			$f2_query  = " INSERT INTO `pms_db_final` (`emp_id`, `yr`, `mos`, `pers_id`, `dim_id`, `pts`, `ratng`, `rmks`  )
   			  			VALUES ('$eid', 2014 , '$mos', '$pers_id' , '$dim_id' , '$pts3' , '$hk_rat', '$hks_rmk')";
